package com.dev.blooddonor.adapter
...
import com.dev.blooddonor.UserDetailsActivity
import com.dev.blooddonor.listener.OnClickListeners
import com.dev.blooddonor.model.User
import kotlinx.android.synthetic.main.item_user.view.*


class AuthorsAdapter(val onClickListeners: OnClickListeners) :
    RecyclerView.Adapter<AuthorsAdapter.AuthorViewModel>() {
    ...
    override fun onBindViewHolder(holder: AuthorViewModel, position: Int) {
        ...
        if (user.profileImage == "") {
            ...
            holder.itemView.profile_image.let {
                Glide.with(it).load(user.profileImage).into(holder.itemView.profile_image)
            }
        }
